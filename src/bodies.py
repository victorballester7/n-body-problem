...
class body_system:  # particle class
    ...
    def __init__(self: 'body_system', name: str, dim: int, days_to_simulate: int, n_bodies: int,
                 bodies_names: np.ndarray, mass: np.ndarray, r0: np.ndarray) -> None:
        self.name = name
        self.dim = dim
        self.days_to_simulate = days_to_simulate
        self.speed_up = 1  # speed up the animation by this factor
    ...
    def integrate_system(self: 'body_system', lib_filename: str,
                         num_steps_max_flow: int,
                         TOL_rk78: float, TOL_coll: float, EPS_field: float) -> None:
        self.num_steps, self.step_size = 0, 0
        if "solar_system_2d" in self.name:
            self.num_steps, self.step_size = param(self.days_to_simulate, 2.)
        else:
            self.num_steps, self.step_size = param(self.days_to_simulate, 0.5)
        ...
        self.r = np.zeros((self.num_steps + 1, 2 * self.dim * self.n_bodies))
        self.com = np.zeros((self.num_steps + 1, self.dim))
